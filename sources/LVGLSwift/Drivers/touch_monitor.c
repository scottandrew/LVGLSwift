...
#include "lvgl_drivers/indev/mouse.h"
#include "lvgl_drivers/indev/keyboard.h"
#include "lvgl_drivers/indev/mousewheel.h"
 ...
 *  STATIC PROTOTYPES
 **********************/
static void touch_monitor_window_create(monitor_t * m);
static void touch_monitor_window_update(monitor_t * m);
int touch_monitor_quit_filter(void * userdata, SDL_Event * event);
static void touch_monitor_sdl_clean_up(void);
static void touch_monitor_sdl_init(void);
static void touch_monitor_sdl_event_handler(lv_task_t * t);
static void touch_monitor_sdl_refr(lv_task_t * t);
...
{
    touch_monitor_sdl_init();
    lv_task_create(touch_monitor_sdl_event_handler, 10, LV_TASK_PRIO_HIGH, NULL);
}
...
//    printf("x1:%d,y1:%d,x2:%d,y2:%d\n", area->x1, area->y1, area->x2, area->y2);
    ...
    for(y = area->y1; y <= area->y2 && y < disp_drv->ver_res; y++) {
        memcpy(&touch_monitor.tft_fb[y * MONITOR_HOR_RES + area->x1], color_p, w * sizeof(lv_color_t));
        color_p += w;
    }
#endif
#endif /*MONITOR_DOUBLE_BUFFERED*/

    touch_monitor.sdl_refr_qry = true;

    /* TYPICALLY YOU DO NOT NEED THIS
     * If it was the last part to refresh update the texture of the window.*/
    if(lv_disp_flush_is_last(disp_drv)) {
        touch_monitor_sdl_refr(NULL);
    }
        ...
        if((&event)->type == SDL_WINDOWEVENT) {
            switch((&event)->window.event) {
                ...
                case SDL_WINDOWEVENT_EXPOSED:
                    touch_monitor_window_update(&touch_monitor);
#if MONITOR_DUAL
                    touch_monitor_window_update(&monitor2);
#endif
                    break;
                default:
                    break;
...
static void touch_monitor_sdl_refr(lv_task_t * t)
{
    ...
    /*Refresh handling*/
    if(touch_monitor.sdl_refr_qry != false) {
        touch_monitor.sdl_refr_qry = false;
        touch_monitor_window_update(&touch_monitor);
    }
...
static void touch_monitor_sdl_clean_up(void)
{
    SDL_DestroyTexture(touch_monitor.texture);
    SDL_DestroyRenderer(touch_monitor.renderer);
    SDL_DestroyWindow(touch_monitor.window);
...
static void touch_monitor_sdl_init(void)
{
    ...
    SDL_SetEventFilter(touch_monitor_quit_filter, NULL);

    touch_monitor_window_create(&touch_monitor);
#if MONITOR_DUAL
    touch_monitor_window_create(&monitor2);
    int x, y;
...
static void touch_monitor_window_create(monitor_t * m)
{
    ...
    SDL_GetDisplayBounds( 1, &displayRect);
